...
using DataDecimals
using DataDecimals: writedecimal!, decimallength, unscaled, scale, _tobigsigned
...
@testset "format" begin
    ...
    for (P, S, T) in ((9, 2, Int32), (18, 4, Int64), (18, 18, Int64), (38, 9, Int128),
        ...
        DT = Decimal{P, S, T}
        lim = _tobigsigned(DataDecimals._maxmag(DT))
        for _ in 1:500
            u = rand(rng, -lim:lim)
            x = reinterpret(DT, DataDecimals._fromfullbig(UInt256, abs(u)) % DataDecimals._utype(T) % T)
            u < 0 && (x = -x)
            @test string(x) == refstring(x)
            @test length(string(x)) == decimallength(x)
