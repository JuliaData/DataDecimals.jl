...
const _TRIM_COMPILE_TIMEOUT_S = Sys.iswindows() ? 900.0 : 600.0
const _TRIM_EXECUTABLE_TIMEOUT_S = Sys.iswindows() ? 120.0 : 60.0
...
function _setup_trim_env()
    ...
    # DataDecimals from the local checkout and adds JuliaC + Parsers.
    ...
    exit_code, output, timed_out = _run_command_with_timeout(
        ...
        timeout_s = 300.0, log_label = "setup"
    )
    ...
    mktempdir() do tmpdir
        cd(tmpdir) do
            ...
            if trim_errors == 0
                ...
                @test !run_timed_out
                @test run_exit == 0
                @test occursin("decimals trim workload passed", run_output)
            else
                @test exit_code != 0
            end
        end
...
@testset "Trim compile" begin
    ...
    elseif _TRIM_PRE_RELEASE
        ...
        trim_workloads = [
            ("decimals_trim_workload.jl", "decimals_trim_workload"),
        ]
