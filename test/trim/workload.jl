using DataDecimals: DecimalValue, Decimal64, Decimal128, rescale, divide
# JuliaC --trim=safe workload: drives the major DataDecimals.jl paths — parsing
...
const D256_40 = Decimal{76, 40, DataDecimals.Int256}
...
function parse_paths()::Nothing
    x = parse(D2, "123.45")
    check(DataDecimals.unscaled(x) == Int64(12345), "parse basic")
    ...
    check(DataDecimals.unscaled(w) == Int128(-98765432100123456789), "parse wide")
    deep = parse(D256_40, "1.0000000000000000000000000000000000000001")
    check(DataDecimals.scale(deep) == 40, "parse 256-tier")
    dv = parse(DecimalValue{Int64}, "0.005")
    check(DataDecimals.unscaled(dv) == Int64(5) && DataDecimals.scale(dv) == Int32(3),
          "parse DecimalValue")
    # Parsers.jl extension (the CSV-adjacent path)
    p = Parsers.parse(D2, "42.42")
    check(DataDecimals.unscaled(p) == Int64(4242), "Parsers.parse")
    buf = codeunits("7.25,8.50")
    v1, pos, rc = Parsers.parsenext(D2, buf, 1, length(buf))
    check(rc == Parsers.RC_OK && DataDecimals.unscaled(v1) == Int64(725),
          "Parsers.parsenext")
    v2 = Parsers.parse(D2, buf, pos + 1, length(buf))
    check(DataDecimals.unscaled(v2) == Int64(850), "Parsers span parse")
    return nothing
end
...
function arithmetic_paths()::Nothing
    ...
    nv = DataDecimals.normalize(parse(D128_10, "1.5000000000"))
    check(DataDecimals.scale(nv) == Int32(1), "normalize strips zeros")
...
function reduction_paths()::Nothing
    ...
    m = v .* v
    check(DataDecimals.scale(m[1]) == 4, "broadcast mul widen")
...
function format_paths()::Nothing
    ...
    pos = DataDecimals.writedecimal!(buf, 1, x)
