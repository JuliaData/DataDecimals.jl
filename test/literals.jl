using DataDecimals: DecimalValue, divide
# Literal macro, normalize, and scientific-notation display.
using DataDecimals
using DataDecimals: DataDecimals as D, unscaled, scale
...
@testset "normalize" begin
    rng = Xoshiro(10001)
    @test DataDecimals.normalize(dec"1.2000") === DecimalValue{Int32}(12, 1)
    @test DataDecimals.normalize(Decimal{18,4}("5.0000")) === DecimalValue{Int64}(5, 0)
    @test DataDecimals.normalize(DecimalValue{Int64}(12000, 0)) === DecimalValue{Int64}(12000, 0)
    @test DataDecimals.normalize(DecimalValue{Int128}(Int128(123) * Int128(10)^30, 33)) ===
          DecimalValue{Int128}(123, 3)
    @test DataDecimals.normalize(zero(DecimalValue{Int64})) === DecimalValue{Int64}(0, 0)
    @test DataDecimals.normalize(DecimalValue{Int64}(-100, 4)) === DecimalValue{Int64}(-1, 2)
    for _ in 1:20000
        u = rand(rng, -10^12:10^12)
        s = rand(rng, 0:20)
        v = DecimalValue{Int64}(u, s)
        n = DataDecimals.normalize(v)
        @test n == v
