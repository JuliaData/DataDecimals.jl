...
using DataDecimals: Int256, _cmpfloat, _cmpfloatbig, _fromfloat, _fromfloat_big,
...
@testset "radix sort and min/max over coefficients" begin
    ...
    for T in (Decimal{9,2,Int32}, Decimal{18,4,Int64}, Decimal{38,9,Int128}, Decimal{76,20,Int256})
        ...
        @test sortperm(v) == sortperm(DataDecimals.unscaled.(v))
        @test sort(v; by=abs) == sort(v; by=x -> abs(DataDecimals.unscaled(x)))
