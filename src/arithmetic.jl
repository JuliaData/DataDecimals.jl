...
"""
    DataDecimals.divide(x, y, mode=RoundNearest)

Divide two decimals, rounding the true quotient at scale `max(scale(x),
scale(y))` with `mode`. `x / y` is exactly `DataDecimals.divide(x, y, RoundNearest)`
...
[`DataDecimals.DecimalValue`](@ref) operands it keeps the promoted storage type and the
runtime scale `max(scale(x), scale(y))`.

```jldoctest
julia> DataDecimals.divide(DataDecimals.Decimal64{2}("1.00"), DataDecimals.Decimal64{2}("3.00"))
0.33

julia> DataDecimals.divide(DataDecimals.Decimal64{2}("1.00"), DataDecimals.Decimal64{2}("3.00"), RoundUp)
0.34
```
"""
