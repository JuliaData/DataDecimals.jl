...
"""
    DataDecimals.decimallength(x) -> Int

Number of bytes [`writedecimal!`](@ref DataDecimals.writedecimal!) produces for
...
"""
    DataDecimals.writedecimal!(buf, pos, x) -> Int
...
ensure [`decimallength(x)`](@ref DataDecimals.decimallength) bytes of room —
...
julia> n = DataDecimals.writedecimal!(buf, 1, DataDecimals.Decimal64{2}("-1.25"))
6
