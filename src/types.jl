...
# Vector{Decimal{P,S,T}} stores a contiguous integer coefficient buffer.
# `DecimalValue{T}` carries the scale per value, as row-oriented wire
# formats do (a PostgreSQL numeric's dscale).

"""
    DataDecimals.AbstractDecimal <: Real

Supertype of the package's exact decimal types, [`Decimal`](@ref) (scale fixed
in the type) and [`DataDecimals.DecimalValue`](@ref) (scale carried per value). Every
...
function _storagetype(P)
    (P isa Int && 1 <= P <= 76) || throw(ArgumentError("precision must be an Int in 1:76"))
    return P <= 9 ? Int32 : P <= 18 ? Int64 : P <= 38 ? Int128 : Int256
end
...
# Validate before any parameter-dependent arithmetic or generated power table.
_validdecparams(P, S, T) = P isa Int && S isa Int && isconcretetype(T) &&
    T <: StorageInt && 1 <= P <= _capacity(T) && 0 <= S <= P

# fold parameter validation to a compile-time constant
@generated function _checkdecparams(::Val{P}, ::Val{S}, ::Type{T}) where {P, S, T}
    _validdecparams(P, S, T) && return :(nothing)
    message = string("invalid Decimal parameters: precision must be ",
...
"""
    Decimal{P, S, T} <: DataDecimals.AbstractDecimal
    Decimal{P, S}
...
`Int32`, `Int64`, `Int128`, or `DataDecimals.Int256` for `P` up to 9, 18, 38, or 76
...
can share an Arrow coefficient buffer of the same width and host byte order.
Other wire formats require their own encoding. See [`DataDecimals.Decimal64`](@ref) and its siblings for the aliases
naming those tiers.
...
[`DataDecimals.rescale`](@ref) and `round(D, x, mode)` are the explicitly-rounding forms.
...
"""
    DataDecimals.Decimal32{S}

Alias for `Decimal{9,S,Int32}` — the widest precision the 32-bit storage tier
holds. See [`DataDecimals.Decimal64`](@ref) for the family.
"""
const Decimal32{S} = Decimal{9, S, Int32}

"""
    DataDecimals.Decimal64{S}
    DataDecimals.Decimal32{S}
    DataDecimals.Decimal128{S}
    DataDecimals.Decimal256{S}

Aliases for the four storage tiers at their maximum precision:

| alias | expands to | digits | bits |
|---|---|---|---|
| `DataDecimals.Decimal32{S}` | `Decimal{9,S,Int32}` | 9 | 32 |
| `DataDecimals.Decimal64{S}` | `Decimal{18,S,Int64}` | 18 | 64 |
| `DataDecimals.Decimal128{S}` | `Decimal{38,S,Int128}` | 38 | 128 |
| `DataDecimals.Decimal256{S}` | `Decimal{76,S,DataDecimals.Int256}` | 76 | 256 |

`S` is the scale, so `DataDecimals.Decimal64{2}` is the usual money type. Use
`Decimal{P,S}` directly when a schema pins a precision narrower than the tier's
maximum.

```jldoctest
julia> DataDecimals.Decimal64{2} === Decimal{18,2,Int64}
true
```
"""
...
Alias for `Decimal{38,S,Int128}`. See [`DataDecimals.Decimal64`](@ref) for the family.
"""
...
Alias for `Decimal{76,S,DataDecimals.Int256}`, the widest tier. See
[`DataDecimals.Decimal64`](@ref) for the family.
"""
const Decimal256{S} = Decimal{76, S, Int256}

"""
    DataDecimals.DecimalValue{T} <: DataDecimals.AbstractDecimal
    DataDecimals.DecimalValue(unscaled, scale)
...
storage integer (`Int32`, `Int64`, `Int128`, or `DataDecimals.Int256`); the bare
`DataDecimals.DecimalValue(u, s)` constructor infers it from `u`, defaulting to `Int64`.
...
where `DataDecimals.DecimalValue(x)` produces the *exact* binary expansion of `x` (that is
what a runtime scale is for) rather than rounding to a fixed scale.
[`DataDecimals.rescale`](@ref) moves a value to another scale.

```jldoctest
julia> DataDecimals.DecimalValue(12345, 3)
12.345

julia> repr(DataDecimals.DecimalValue(0.1))
"DataDecimals.DecimalValue{DataDecimals.Int256}(\\"0.1000000000000000055511151231257827021181583404541015625\\")"
```
"""
...
"""
    DataDecimals.unscaled(x) -> Integer
...
"""
    DataDecimals.scale(x) -> Int
    DataDecimals.scale(::Type{<:Decimal}) -> Int
...
[`DataDecimals.DecimalValue`](@ref). The companion precision is `Base.precision`, which is
...
@generated function _maxmag(::Type{Decimal{P, S, T}}) where {P, S, T <: StorageInt}
    _validdecparams(P, S, T) || return :(_checkdecparams(Val($(QuoteNode(P))), Val($(QuoteNode(S))), T))
...
# A fractional-only type cannot hold ±1. Return the coefficient's integer
# sign for every decimal so this operation is total and type-stable.
Base.sign(x::AbstractDecimal) = sign(x.unscaled)
