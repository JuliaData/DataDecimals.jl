...
beyond 76 digits of precision fall back to a `DataDecimals.DecimalValue{Int256}`. Digit
...
(`DataDecimals.Decimal64{2}("1.25")`, or `convert`) when a column type is wanted.
"""
macro dec_str(s)
...
"""
    normalize(x) -> DataDecimals.DecimalValue
...
scale 0): `normalize(dec"1.2000") == DataDecimals.DecimalValue(12, 1)`.

The result is always a [`DataDecimals.DecimalValue`](@ref) — the scale is a property of the
