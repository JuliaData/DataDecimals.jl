...
| `/` | rounds half-even at scale `max(S1, S2)`; [`DataDecimals.divide(x, y, mode)`](@ref DataDecimals.divide) for any other mode |
...
| `DataDecimals.DecimalValue(::AbstractFloat)` | the exact binary expansion, which is what a runtime scale is for |
...
| [`DataDecimals.rescale`](@ref), `round(D, x, mode)` | the explicit rounding conversions between decimal types |
...
- [`DataDecimals.divide(x, y, mode)`](@ref DataDecimals.divide) instead of `/`
- [`DataDecimals.rescale(D, x, mode)`](@ref DataDecimals.rescale), [`DataDecimals.rescale(v, s, mode)`](@ref DataDecimals.rescale)
...
| `DataDecimals.DecimalValue{T1}` ⊕ `DataDecimals.DecimalValue{T2}` | `DataDecimals.DecimalValue{promote_type(T1,T2)}`, scale `max` |
| `DataDecimals.DecimalValue` ⊕ `Decimal` | `DataDecimals.DecimalValue` with the promoted storage type |
...
`Integer(DataDecimals.Decimal64{2}("5.00"))` is an `Int64` and `Integer(DataDecimals.Decimal256{2}("5.00"))`
is a `DataDecimals.Int256`. Ask for `Int(x)` when you want an `Int`.

Integer, rational, and decimal sources convert exactly or throw. Float
construction and conversion, string construction, and `parse` round half-even
at the target scale. Runtime-scale float construction is exact or throws.

## Equality, ordering, and hashing

Comparison is exact and cross-type. `dec"1.20"`, `dec"1.2"`, `12//10`, and
`DataDecimals.Decimal128{9}("1.200000000")` all compare equal, and all hash equal.
...
`string`, `print`, and [`writedecimal!`](@ref DataDecimals.writedecimal!) always
...
scientific notation beyond it, so a scale-1000 `DataDecimals.DecimalValue` does not fill a
...
`Printf`'s `%f`/`%e`/`%g` round in decimal before conversion to a sufficiently
precise `BigFloat`; `%d` is exact-or-`InexactError`. `JSON.json` emits exact digits.
