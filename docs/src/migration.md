# Migration from Decimals.jl

DataDecimals 1.0 is a separate fixed-point package. Decimals.jl 0.x remains
a separate arbitrary-precision floating-point package. To migrate, replace
`using Decimals` with `using DataDecimals` and review the arithmetic below.
...
[`DataDecimals.DecimalValue{T}`](@ref DataDecimals.DecimalValue) covers the row-oriented case where the
...
- `DataDecimals.normalize` for stripping trailing zeros.
...
| 0.x | 1.0 |
|---|---|
| `Decimal(1.25)`, one untyped decimal | choose a type: `DataDecimals.Decimal64{2}("1.25")` for a fixed-scale column, [`DataDecimals.DecimalValue`](@ref) when the scale is per value, `dec"1.25"` for a literal |
| silent rounding to context precision on `+`/`-`/`*` | exact and checked; opt into rounding with [`DataDecimals.rescale(D, x, mode)`](@ref DataDecimals.rescale) or [`DataDecimals.divide(x, y, mode)`](@ref DataDecimals.divide) |
...
| `precision(x)` = the context's digit budget | `precision(x)` = the type's `P`; `DataDecimals.scale(x)` for the scale |
...
```julia
# 1.0
DataDecimals.divide(DataDecimals.rescale(DataDecimals.Decimal64{10}, a),
                DataDecimals.rescale(DataDecimals.Decimal64{10}, b), RoundUp)
```
...
total = DataDecimals.rescale(DataDecimals.Decimal64{2}, a * b + c) # half-even into 2 decimal places
```
...
- The `DataDecimals.Decimal32` / `DataDecimals.Decimal64` / `DataDecimals.Decimal128` / `DataDecimals.Decimal256` tiers, with
  precision and scale carried in the type.
- [`DataDecimals.DecimalValue`](@ref) for runtime scales.
- The zero-copy wire API: `DataDecimals.unscaled`, `DataDecimals.scale`,
  `reinterpret` construction, [`writedecimal!`](@ref DataDecimals.writedecimal!),
  and [`decimallength`](@ref DataDecimals.decimallength). Arrow buffers can share
  the same layout; other wire formats require their own encoding.
- The explicit rounding API: [`DataDecimals.rescale`](@ref), [`DataDecimals.divide`](@ref), and
...
LibPQ.jl pins `DataDecimals = 0.4.1`, a pre-0.5 API that already stopped working
with 0.5.x, so 1.0 is no worse for it, and [`DataDecimals.DecimalValue`](@ref) is the
